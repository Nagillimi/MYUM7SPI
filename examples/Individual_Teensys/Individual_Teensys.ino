/* Arduino example for a Teensy Low Latency Logger using DEDICATED SPI
   ...
   Ben Milligan, September 2020
  
   An implementation of the UM7's SPI mode to allow for logging of 
   1 UM7 @ 250 Hz (will duplicate data) and 2 FSR's @ 500 Hz
   (latency of 2000 usec)into Bill Greiman's SdFat-beta library 
   ExFatLogger example. Thanks Bill!!

   This example overcomes the overflow errors seen in longer SPI transmissions 
   by using multiple Teensy controllers set at the imus. The power and start
   button cables will be passed between each, allowing a synchronous start
   time between all controllers.
...
void binaryToCsv() {
  Serial.println(F("Converting binary data..."));
  uint8_t lastPct = 0;
...
void logData() {
  ...
  Serial.println();
  Serial.println(F("Hit button to start logging..."));
  // Wait for the start button to be pushed to synchronize Teensy's
  ...
  while (true) {
    ...
    if (!sd.card()->isBusy()) {
      ...
      // If start button is pushed again, end trial
...
void loop() {
  ...
  // For printData(), clarifying if you want to test or not (to Serial)
  test = false;
  
  createBinFile();
  // Button prompt is inside logData to allow the best sync between Teensy's
  logData();
  // Convert file once done.
  if (createCsvFile())
      binaryToCsv();
